<template>
  <div class="rounded-xl border border-gray-200 bg-white shadow-sm">
    ...
    <div class="p-5 space-y-4">
      <!-- N° Recibo -->
      <div>
        <label class="block text-xs font-semibold text-gray-600 mb-1">N° Recibo *</label>
        <input
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">Valor base</label>
          <div class="relative">
        ...
        <div>
          <label class="block text-xs font-semibold text-gray-600 mb-1">Impuesto</label>
          <div class="relative">
            <span class="absolute inset-y-0 left-3 flex items-center text-gray-400 text-sm">$</span>
            <input
      ...
      <div class="grid grid-cols-2 gap-3">
        <div>
          ...
          <input v-model="form.fecha_emision" type="date" :class="fieldClass()" />
        </div>
        <div>
...
import { useCobranzaForm, CONCEPTOS_COBRANZA, MESES, formatCurrency } from '../composables/useCobranza'
...
function recalcularTotal() {
  ...
  if (base > 0) form.value.valor_total = base + impuesto
}
