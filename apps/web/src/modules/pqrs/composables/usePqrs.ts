...
export function estadoBadgeClass(estado: EstadoPqrs): string {
  const map: Record<EstadoPqrs, string> = {
    ...
    'en proceso': 'bg-yellow-100 text-yellow-800 ring-yellow-200',
...
export function usePqrs() {
  ...
  return {
    store, TIPOS_PQRS, ESTADOS_PQRS, PRIORIDADES_PQRS, CATEGORIAS_PQRS,
    estadoBadgeClass, prioridadBadgeClass, tipoLabel, tipoIcon, formatDateTime, formatDate,
  }
}
// ─── Composable formulario ────────────────────────────────────────────────────
export function usePqrsForm(conjuntoId?: string, unidadId?: string) {
  const store = usePqrsStore()
  ...
  function reset() {
    form.value = {
      conjuntoId: conjuntoId ?? '', unidadId: unidadId ?? '',
      tipo: 'peticion', asunto: '', descripcion: '', prioridad: 'normal',
...
export function usePqrsGestion() {
  ...
  const tomarCaso = (id: string, nombre: string) =>
    cambiarEstado(id, { estado: 'en proceso', responsable_asignado_nombre: nombre })
